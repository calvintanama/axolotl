...
class Phi3WithExtraModuleAttention(Phi3Attention):
    def __init__(self, config: Phi3WithExtraModuleConfig, layer_idx: Optional[int] = None):
        super().__init__(config, layer_idx)
        if config.extra_module == "lora":
            self.extra_module = config.extra_module
            op_size = self.num_heads * self.head_dim + 2 * (self.num_key_value_heads * self.head_dim)
            self.qkv_proj = LoRAMatrices(self.hidden_size, op_size, r=config.r)
            self.reset_extra_module()

    def reset_extra_module(self):
        if self.extra_module == "lora":
            nn.init.zeros_(self.qkv_proj.lora_a.weight)
            nn.init.normal_(self.qkv_proj.lora_b.weight)
...
class Phi3WithExtraModuleDecoderLayer(Phi3DecoderLayer):
    def __init__(self, config: Phi3WithExtraModuleConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        self.self_attn = Phi3WithExtraModuleAttention(config, layer_idx)
...
class Phi3WithExtraModuleForCausalLM(Phi3ForCausalLM):
    config_class = Phi3WithExtraModuleConfig
    ...
    def insert_extra_layers(self, extra_module: str, num_extra_module: int, prune_start_index: int, prune_end_index: int, r: int = None, residual: bool = None, residual_type: str = None):
        ...
        self.config._extra_module_validation()

        warning_msg = """
            Inserting extra {0} layers of {1} module in pruned layer {2} and {3}
        """
